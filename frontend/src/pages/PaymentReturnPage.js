...
import { FaCheckCircle, FaTimesCircle, FaHome, FaReceipt, FaCreditCard, FaCalendarAlt, FaUniversity, FaMoneyBillWave, FaRedo } from 'react-icons/fa';
...
function PaymentReturnPage() {
    ...
    return (
      <div className="paymentreturnpage-container">
        <div className="paymentreturnpage-card error-card">
          ...
          <h2 className="paymentreturnpage-title">Không có dữ liệu kết quả thanh toán.</h2>
          <p className="error-message">Đã xảy ra lỗi khi xử lý giao dịch của bạn.</p>
  ...
  const paymentProcessUrl = `http://localhost:3000/payment-process/${paymentResult.paymentId}`;

  return (
    <div className="paymentreturnpage-container">
      {showConfetti && <Confetti />}
      <div className="paymentreturnpage-card">
        <div className={`paymentreturnpage-receipt ${paymentResult.status === 'SUCCESS' ? 'success-receipt' : 'failure-receipt'}`}>
          ...
          <div className="receipt-body">
            <div className="receipt-amount">
              {paymentResult.amount && (
                <>
                  ...
                  <span className="amount-value">{paymentResult.amount.toLocaleString('vi-VN')} ₫</span>
                </>
              )}
            </div>
            ...
            <div className="receipt-details">
              <div className="detail-row">
                <div className="detail-icon"><FaReceipt /></div>
                <div className="detail-label">Mã đơn hàng:</div>
                <div className="detail-value">{paymentResult.orderId || "N/A"}</div>
              </div>
              ...
              {paymentResult.transactionNo && (
                <div className="detail-row">
                  ...
                  <div className="detail-value">{paymentResult.transactionNo}</div>
                </div>
              )}
              ...
              {paymentResult.orderInfo && (
                <div className="detail-row">
                  ...
                  <div className="detail-value">{decodeURIComponent(paymentResult.orderInfo)}</div>
                </div>
              )}
              ...
              {paymentResult.formattedPayDate && (
                <div className="detail-row">
                  ...
                  <div className="detail-value">{paymentResult.formattedPayDate}</div>
                </div>
              )}
          ...
          <div className="receipt-footer">
            <div className="receipt-actions">
              ...
              {paymentResult.status === 'SUCCESS' ? (
                <button onClick={() => window.print()} className="paymentreturnpage-button secondary-button">
                  <FaReceipt className="button-icon" /> In biên lai
                </button>
              ) : (
                <a href={paymentProcessUrl} className="paymentreturnpage-button secondary-button">
