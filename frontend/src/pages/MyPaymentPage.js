...
import { Calendar, DollarSign, ChevronDown, ChevronUp, User, BookOpen, CreditCard, CheckCircle, XCircle, Download } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const paymentsResponse = await fetch(`http://localhost:8080/payment/getbycustomerid?customerId=${userInfo.userId}`, {
          method: 'GET',
          credentials: 'include',
        });
        ...
        const sortedPayments = paymentsData.sort((a, b) => new Date(a.date) - new Date(b.date));
    ...
    <div className="mypayment-container">
      ...
      <div className="mypayment-content">
        <div className="mypayment-sidebar">
          <div
            className={`mypayment-sidebar-item ${activeTab === 'profile' ? 'active' : ''}`}
          ...
          <div
            className={`mypayment-sidebar-item ${activeTab === 'children' ? 'active' : ''}`}
          ...
          <div
            className={`mypayment-sidebar-item ${activeTab === 'bookings' ? 'active' : ''}`}
          ...
          >
            ...
            <button className="mypayment-dropdown-toggle" onClick={toggleDropdown}>
          ...
          <div
            className={`mypayment-sidebar-item ${activeTab === 'payments' ? 'active' : ''}`}
                ...
                <table className="mypayment-table">
                  ...
                  <tbody>
                    {filteredPayments.map((payment, index) => {
                      const isExpanded = payment.paymentId === expandedPaymentId;
                      const statusClass = payment.status ? 'active' : 'inactive';
                      ...
                      return (
                        <>
                          <tr
                            ...
                            onClick={() => togglePaymentDetail(payment.paymentId)}
                          >
                            <td>{index + 1}</td>
                            ...
                            <td>
                              <span className={`mypayment-status ${statusClass}`}>
                                {payment.status ? 'Đã thanh toán' : 'Chưa thanh toán'}
                              </span>
                            </td>
                            <td>{payment.method ? 'Chuyển khoản' : 'Tại quầy'}</td>
                            <td>
                              {!payment.status && (
                                <button
                                ...
                                <div className="mypayment-card-details">
                                  <div className="mypayment-detail-section">
                                    ...
                                    <div className="mypayment-detail-item">
                                      <span className="mypayment-detail-label">Booking ID:</span>
                                      <span className="mypayment-detail-value">{payment.booking.bookingId}</span>
                                    </div>
                                    <div className="mypayment-detail-item">
                                      <span className="mypayment-detail-label">Ngày Booking:</span>
                                      <span className="mypayment-detail-value">{new Date(payment.booking.bookingDate).toLocaleDateString('vi-VN')}</span>
                                    </div>
                                  </div>
                                  <div className="mypayment-detail-section">
                                    <h4>Thông Tin Thanh Toán</h4>
                                    <div className="mypayment-detail-item">
                                      <span className="mypayment-detail-label">Hóa đơn ID:</span>
                                      <span className="mypayment-detail-value">{payment.paymentId}</span>
                                    </div>
                                    <div className="mypayment-detail-item">
                                      <span className="mypayment-detail-label">Ngày thanh toán:</span>
                                    ...
                                    <div className="mypayment-detail-item">
                                      <span className="mypayment-detail-label">Tổng tiền:</span>
                                      <span className="mypayment-detail-value">{payment.total.toLocaleString('vi-VN')} VNĐ</span>
                                    </div>
                                    <div className="mypayment-detail-item">
                                      <span className="mypayment-detail-label">Phương thức:</span>
                                      <span className="mypayment-detail-value">{payment.method ? 'Chuyển khoản' : 'Tại quầy'}</span>
                                    </div>
                                    <div className="mypayment-detail-item">
                                      <span className="mypayment-detail-label">Trạng thái:</span>
                                      <span className={`mypayment-status ${statusClass}`}>
                                        {payment.status ? 'Đã thanh toán' : 'Chưa thanh toán'}
                                      </span>
                                    </div>
                                    <div className="mypayment-detail-item">
                                      <span className="mypayment-detail-label">Mã giao dịch:</span>
                                      <span className="mypayment-detail-value">{payment.transactionId || 'Không có'}</span>
                                    </div>
                                    <div className="mypayment-detail-item">
                                      <span className="mypayment-detail-label">Chiến dịch tiếp thị:</span>
                                      <span className="mypayment-detail-value">
                                        {payment.marketingCampaign && payment.marketingCampaign.coupon
