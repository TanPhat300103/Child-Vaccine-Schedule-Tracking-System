...
import { FiCalendar, FiUsers, FiAlertTriangle, FiChevronDown, FiChevronUp } from "react-icons/fi";
...
const StaffProfile = ({ initialStaffData }) => {
  ...
  const handleChange = (e) => {
    ...
    const changed = Object.keys(updatedData).some((key) => {
      if (key === "staffId" || key === "roleId" || key === "active") return false;
...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const fetchStaffData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`http://localhost:8080/staff/findid?id=${staffId}`, {
          method: "GET",
          credentials: "include",
        });
  ...
  if (loading) return <div className="text-center text-gray-600 py-12">Đang tải dữ liệu...</div>;
  if (error) return <div className="text-red-600 p-4 text-center">{error}</div>;
  if (!staffData) return <div className="text-gray-600 p-4 text-center">Không có dữ liệu</div>;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-2xl shadow-md p-6 border-l-4 border-blue-400 hover:shadow-lg transition-all ease-in-out">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-500">Vaccine có sẵn</p>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6 border-l-4 border-red-400 hover:shadow-lg transition-all ease-in-out">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-500">Báo cáo phản ứng</p>
