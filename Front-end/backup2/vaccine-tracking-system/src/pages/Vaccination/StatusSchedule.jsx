...
const VaccinationSchedule = () => {
  ...
  const handleViewDetail = (booking) => {
    navigate("/vaccine-page2", { state: booking });
  };
      ...
      <div className="container mx-auto px-4 py-8">
        <header className="text-center mb-8">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <h1 className="text-3xl font-bold text-gray-800">Lịch Tiêm Chủng</h1>
          </div>
        ...
        <div className="mb-6">
          <div className="flex justify-center space-x-4 border-b border-gray-200">
            {["completed", "pending", "upcoming"].map((tab) => (
              ...
              >
                {tab === "completed" ? "Đã tiêm" : tab === "pending" ? "Chờ xác nhận" : "Sắp tiêm"}
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredBookings.length > 0 ? (
              filteredBookings.map((booking) => (
                <div key={booking.bookingDetailId} className="bg-white rounded-lg shadow-md p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-800">
                      {booking.vaccine.name} {/* Hiển thị tên vaccine */}
                    </h3>
                    {getStatusIcon(getVaccineStatus(booking.status))}
                  </div>
                  <p className="text-gray-600 text-sm mb-2">Trạng thái: {getVaccineStatus(booking.status)}</p>
                  <p className="text-gray-500 text-sm">Người đăng ký: {booking.booking.customer.firstName} {booking.booking.customer.lastName}</p>
                  <p className="text-gray-500 text-sm">Trẻ em: {booking.child.firstName} {booking.child.lastName}</p>
                  <p className="text-gray-500 text-sm">Ngày tiêm dự kiến: {formatDate(booking.scheduledDate)}</p>
                  <button
